...
feup <- quantile(df_osm_edge$points, 0.98)
...
Traffic  Map April 2016 com gs filter
```{r}
m 
```
...
df_intersession_april_gs <- df_speed %>% 
...
df_intersession_april_gs$time <- as.POSIXct(df_intersession_april_gs$seconds, origin="1970-01-01")

df_intersession_april_gs <- df_intersession_april_gs  %>%
...
```{r echo=FALSE}
plot_ly(y = df_intersession_april_gs$intersession_time, type = "box", name = "All way_ids") %>% 
  add_boxplot(y= filter(df_intersession_april_gs, intersession_time > 0 )$intersession_time, name = "At least 2 sessions") %>% 
  layout(title="Intersession time (mins)")
...
### ECDF 
```{r echo=FALSE}
e1 <- ggplot(subset(df_intersession_april, intersession_time > 0), aes(intersession_time, xmax = 600)) + 
  stat_ecdf(geom = "step") +  xlab("Intersession time (mins)") 
...
df_intersession_april %>% 
  ...
  xlab("Media intersession time (min) ")
...
```{r}
prueba <- table(cut(filter(df_speed, day(time) < 7)$time, breaks = "30 mins"))
...
```{r}
filter(df_speed, way_id == 37141967 ) %>% 
...
```{r}
table(cut(df_speed, way_id == 37141967 )$time, breaks = "30 mins"))
```
