...
df_segments_length <- dbGetQuery(con, "SELECT session_id, SUM(length) as distance, travelmode
...
```{r}
df_segments <- subset(df_segments_length, distance < mean(distance))

ggploy(df_segments_length, aes(travelmode, distance)) +
  geom_bar()
```
...
df_segments_timestamp <- dbGetQuery(con, "SELECT *
                                      FROM danielasocas.segments_timestamp
                                      LIMIT 500")
```


```{r}
df_segments_timestamp <- mutate(df_segments_timestamp, date= ymd_hms(timestamp, tz="Europe/Lisboa"),
                                WeekDay=wday(timestamp),nWeekDay=wday(timestamp,label=TRUE), time = hms(timestamp))
