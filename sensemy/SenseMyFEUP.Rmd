...
df_points_edge_cars_april16 <- dbGetQuery(con, "SELECT way_id, COUNT(*) AS points
                                       FROM sensemyfeup_raw.osmlocation as osm 
                                          ...
                                          INNER JOIN sensemyfeup_raw.session_offsets 
                                            USING (session_id)
                                       WHERE (osm.seconds + session_offsets.clock_offset_seconds BETWEEN segments.seconds_start AND seconds_end) AND
                                            (segments.travelmode = 'car' OR segments.travelmode =  'bus') AND 
...
df_points_edge_cars_april16_day <- dbGetQuery(con, "SELECT way_id, COUNT(*) AS points
                                       ...
                                       WHERE (osm.seconds + session_offsets.clock_offset_seconds BETWEEN segments.seconds_start AND seconds_end) AND
                                            (segments.travelmode = 'car' OR segments.travelmode =  'bus') AND 
                                            EXTRACT(MONTH from time_start) = 4 AND EXTRACT(YEAR from time_start) = 2016 AND
EXTRACT(HOUR from time_start) >= 7 AND EXTRACT(HOUR from time_start) < 20
                                       GROUP BY way_id
                                       ORDER BY points DESC")
...
df_points_edge_cars_april16_night <- dbGetQuery(con, "SELECT way_id, COUNT(*) AS points
                                       ...
                                       WHERE (osm.seconds + session_offsets.clock_offset_seconds BETWEEN segments.seconds_start AND seconds_end) AND
                                            (segments.travelmode = 'car' OR segments.travelmode =  'bus') AND 
                                            EXTRACT(MONTH from time_start) = 4 AND EXTRACT(YEAR from time_start) = 2016 AND
EXTRACT(HOUR from time_start) >= 20 OR EXTRACT(HOUR from time_start) < 7
                                       GROUP BY way_id
                                       ORDER BY points DESC")
...
df_first_session_cars_april16 <- dbGetQuery(con, "SELECT distinct on (session_id) session_id, time_start,time_end
                                          FROM danielasocas.segments_travelmode AS segments
...
df_intersession_april <- data.frame()
df_intersession_april <- dbGetQuery(con, "SELECT session_id, way_id, MIN(seconds) as seconds
                                                  FROM sensemyfeup_raw.osmlocation AS osm
                                                  INNER JOIN danielasocas.segments_vehicules_april AS segments
	                                                  USING (session_id)
                                                  INNER JOIN sensemyfeup_raw.session_offsets 
	                                                  USING (session_id)
                                                  WHERE (osm.seconds + session_offsets.clock_offset_seconds BETWEEN segments.seconds_start AND seconds_end)
                                                  GROUP BY way_id, session_id
                                                  ORDER BY way_id")

save(df_intersession_april, file="df_intersession_april.Rda")
