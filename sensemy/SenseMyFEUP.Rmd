...
```{r echo= FALSE}
df_segments <- subset(df_segments_length, distance < (distance,0.75) )
...
```{r}
df_wayids_porto_april <- data.frame()
df_wayids_porto_april <- dbGetQuery(con, "SELECT session_id, segment_id, way_id, osm.seconds, osm.lat, osm.lon, gs.speed
...
save(df_wayids_porto_april, file="df_wayids_porto_april.Rda")
```
...
df_speed_top <- dbGetQuery(con, "SELECT session_id, segment_id, way_id, osm.seconds, osm.lat, osm.lon, gs.speed, gs.track
                            ...
                            WHERE (osm.seconds between segments.seconds_start AND segments.seconds_end) AND
gs.seconds = osm.seconds AND gs.millis = osm.millis AND 
(alt != 0  OR  track != 0) AND (way_id = 35078500 OR way_id = 35145521 OR way_id = 316459169 OR way_id = 37141967 OR way_id = 63192525 OR way_id = 106334851 OR way_id = 37141970 OR way_id = 36979228) ")
