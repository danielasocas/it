...
dfm_base <- dfm_base %>% 
  mutate( period = case_when(hour == 8 | 
                               ...
                               hour == 18 |
                               hour == 19 |
                               hour == 20  ~ factor(hour, levels = c(8,9,10,11,18,19,20,"midday","afternoon")), 
                             hour == 12 |
                               hour == 13 |
                               hour == 14  ~ as.factor("midday"), 
                             hour == 15 |
                               hour == 16 |
                               hour == 17 ~ as.factor("afternoon")))
...
(mae <- mean(abs(dfm_test$speed - predmars)))
summary(predmars)
